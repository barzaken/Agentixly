    ...
    <div>
      ...
      <Container className="border-divide flex flex-col items-center border-x pt-20 md:pb-10">
        ...
        <SubHeading className="mx-auto mt-2 max-w-sm px-4">
          all blogs written by the Agentix team
        </SubHeading>
...
export const GridLayout = ({ blogs }: { blogs: any[] }) => {
  return (
      ...
      {blogs.map((blog, index) => (
        ...
        >
          <Image
            ...
            width={500}
            height={500}
            className="shadow-agentix h-60 w-full rounded-lg object-cover md:h-80 lg:h-60"
          />
          <div>
...
export const RowLayout = ({ blog }: { blog: any }) => {
  return (
    <Link
      key={blog.title}
      href={`/blog/${blog.slug}`}
